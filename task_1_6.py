...
class GarbageInQueueException(Exception):
...
class TaskBoard:
    ...
    def to_base_queue(self, obj: Task):
        try:
            if type(obj) != Task:
                raise GarbageInQueueException('Попытка добавить некорректную задачу!')
            self.base_queue.insert(0, obj)
            print(f'{self.base_queue[0].description} - добавлен в основную очередь задач')
        except GarbageInQueueException:
            print('Ошибка, в очередь можно поместить только объект класса Task!')
